...
# anonymization data visulization -----------------------------------------
...
posts_table <- posts_table %>% na.omit() %>% filter(postAuthor != "康佩如")

anonymization <- LETTERS[1:18]
names(anonymization) <- unique(posts_table$postAuthor)

posts_table$postAuthor_anony <- anonymization[posts_table$postAuthor]
posts_table$replyBy_anony <- anonymization[posts_table$replyBy]
...
# barplot: number of posts ------------------------------------------------
  ...
  students_freq <- students_posts_table %>% group_by(postAuthor, postAuthor_anony) %>% count()
...
# barplot: number of times reply to others --------------------------------
  ...
  replyToOther_freq <- students_posts_table %>%
      ...
      group_by(replyBy, replyBy_anony) %>%
...
# positive reply by professor ---------------------------------------------
  ...
  positivie_freq <- positive_table %>% group_by(postAuthor, postAuthor_anony) %>% count() %>% arrange(desc(n))
...
post_freq_student_byDate <- students_posts_table  %>% 
  group_by(date, postAuthor, postAuthor_anony) %>% 
  count() %>% 
  na.omit()
...
ggplot(data = post_freq_student_byDate[post_freq_student_byDate$postAuthor_anony == "O",],
...
replyToOther_byDate <- students_posts_table %>%
  ...
  group_by(replyBy, date, replyBy_anony) %>%
  count()

ggplot(data = replyToOther_byDate[replyToOther_byDate$replyBy_anony == "O",],
       mapping = aes(x = date, y = n))  +
  geom_point() + 
  geom_line(aes(color = "selected student")) +
  scale_x_date(date_breaks = "1 week") +
